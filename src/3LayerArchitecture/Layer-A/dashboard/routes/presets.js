...
module.exports = (client) => {
    router.post('/add', checkAuth, async (req, res) => {
        ...
        await addVoicePreset(userId, name, {
    ...
    router.post('/update', checkAuth, async (req, res) => {
        ...
        await updateVoicePreset(userId, presetId, name, newSettings);
        res.redirect(`/dashboard/${guildId}`);
    });

    router.post('/delete', checkAuth, async (req, res) => {
        const { presetId } = req.body;
        await deleteVoicePreset(presetId, req.user.id);
        res.redirect(`/dashboard/${req.params.guildId}`);
    });

    router.post('/load', checkAuth, async (req, res) => {
        const { guildId } = req.params;
        const userId = req.user.id;
        const { presetId } = req.body;
        ...
        if (preset && preset.user_id === userId) {
            await setUserSpeakerId(guildId, userId, preset.speaker_id, preset.speaker_type);
            await setUserSpeed(guildId, userId, preset.speed);
            await setUserPitch(guildId, userId, preset.pitch);
