...
module.exports = () => {
    router.post('/add', checkGuildAdmin, async (req, res) => {
        const { word, read } = req.body;
        if (word && read) await addDictionaryEntry(req.params.guildId, word, read);
        res.redirect(`/dashboard/${req.params.guildId}#dictionary`);
    });

    // ★ 修正: IDがあればIDで削除、なければ単語で削除
    router.post('/delete', checkGuildAdmin, async (req, res) => {
        if (req.body.id) {
            await removeDictionaryEntryById(req.params.guildId, req.body.id);
        } else if (req.body.word) {
            await removeDictionaryEntry(req.params.guildId, req.body.word);
        }
        res.redirect(`/dashboard/${req.params.guildId}#dictionary`);
    });

    // ★ インポート処理 (新規追加)
    router.post('/import', checkGuildAdmin, upload.single('dictFile'), async (req, res) => {
        ...
        try {
            ...
            // DBへ登録
            const count = await importDictionary(guildId, entries);
    ...
    router.get('/export', checkGuildAdmin, async (req, res) => {
