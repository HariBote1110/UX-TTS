...
async function getAudioResource(text, speakerId, speed, pitch) {
    ...
    // 1. キャッシュ確認
    const cachedFilePath = await getCache(hash, speakerId, speed, pitch);
    ...
    try {
        ...
        await insertCache(hash, speakerId, speed, pitch, filePath, fileSize);
    ...
    try {
        ...
        for (const entry of entries) {
            if (fs.existsSync(entry.file_path)) {
                ...
                await deleteCacheEntry(entry.file_path);
            }
        }
